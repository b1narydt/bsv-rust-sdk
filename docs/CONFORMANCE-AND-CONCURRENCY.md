...
| Non-ASCII certificate field-name collation differs from Node/ICU | 1 | The Rust comparator pins 401 Node-generated ASCII orderings, but deliberately gives non-ASCII characters weights above ASCII instead of attempting to reproduce host-dependent ICU collation. Legal non-ASCII field names can therefore produce different certificate binary/signature preimages across SDKs. Verification fails closed, real issuance paths use ASCII identifier names, and rewriting the comparator would risk the pinned ASCII domain | Live `@bsv/sdk` 2.4.1 observation (`aaaa, éééé, tag_A, tag-A, zzzz`) versus the documented Rust order; 401 ASCII comparator vectors remain normative; consider enforcing ASCII field names in a future API change |
| Non-ASCII `discoverByAttributes` string lengths use UTF-8 bytes | 1 | TS writes UTF-16 code-unit counts (`attrKey.length` and the same expression for values) followed by UTF-8 bytes, so its frame is self-inconsistent for non-ASCII text. Rust writes the actual UTF-8 byte lengths and deliberately does not mirror the malformed frame; ASCII is identical | Serializer site comment; real-TS ASCII insertion-order vector |
| `listCertificates` verifier is encoded as UTF-8 | 1 | TS processor writes `cert.verifier` as hex but its transceiver reads those bytes as UTF-8, making TS asymmetric with itself. Rust matches the reader so callers receive the same verifier text they supplied | Serializer/deserializer site comments; UTF-8 round-trip regression plus legacy TS hex-byte decode regression |
...
Where TS is wrong we conform by default and report upstream. Any exception must be registered above;
an unregistered Rust-only divergence is worse than a shared bug, because no Rust↔Rust test can detect it.
