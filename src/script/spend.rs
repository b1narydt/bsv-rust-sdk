...
mod tests {
    ...
    fn test_op_return_terminates_successfully() {
        // Post-Genesis BSV semantics (matching dxs production engine and the
        // canonical 2,899-byte STAS-3 covenant body): top-level OP_RETURN is
        // a successful early termination. The script's stack at that moment
    ...
    /// for `input_index == 0`. The 6-of-12 P2PKH conformance failures at
    /// input 1 surfaced the bug.
    #[test]
    fn sighash_preimage_orders_by_input_index() {
        ...
        let mut spend = Spend::new(SpendParams {
            ...
            // Two outputs from the same vector
            other_outputs: vec![
                crate::transaction::transaction_output::TransactionOutput {
                    satoshis: Some(100),
                    locking_script: LockingScript::from_hex(
                        // The 2986-byte STAS-3 covenant output script — referenced from the
                        // vector. Inlined here to keep the test hermetic.
                        // We only need the bytes to feed into hashOutputs; the value matches
                        // the reference preimage.
        ...
        // Verify the structural invariant: the outpoint at the current-input
        // section (bytes 68..100 in BIP-143 layout) is THIS input's outpoint.
        // hashPrevouts (bytes 4..36) must hash [input0_outpoint, input1_outpoint],
        // not [input1_outpoint, input0_outpoint].
