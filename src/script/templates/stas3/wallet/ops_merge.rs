//! `Stas3Wallet::merge` — 2..=7 input STAS-3 merge.
...
impl<W: WalletInterface> Stas3Wallet<W> {
    /// Build a merge tx with 2..=7 STAS inputs (spec §8.1). Each input must
    /// carry `source_tx_bytes` so the factory can derive the per-input piece
    /// array (spec §9.5).
    ///
    ...
    pub async fn merge(
        ...
        if !(2..=7).contains(&tokens.len()) {
            return Err(Stas3Error::InvalidScript(format!(
                "merge requires 2..=7 STAS inputs (spec §8.1); got {}",
                tokens.len()
            )));
        }
