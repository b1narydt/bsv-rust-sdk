...
| 40 | Transport receive/dispatch | Background `onData` callbacks | **Background goroutine** `go t.receiveMessages()` (`websocket_transport.go:74`) | **FIXED** — peer-owned background receiver; nonce-routed handshake waiters; 64-general global / 8-general per-session / 16-control admission; bounded error observer with frame type/request-ID metadata | Implemented on `feat/background-receive-task`. `process_next`/`process_pending`, the pull mutex, deferred queue, and pending-response net are deleted; concurrency width, cross-session fairness, lifetime shutdown, and the certificate-release deadlock are mutation-proven |
