...
Branch `feat/background-receive-task`. Every finding from the adversarial reviews, with status.
...
| R8-2 | Public single-frame `process_next` swallowed its consumed frame's dispatch error | **SUPERSEDED** — both pull APIs were deleted with the background receiver. Direct `dispatch_message` returns its owned error; background errors are non-blocking per-frame events, and AuthFetch correlates response-frame errors by request ID |
...
| F2 / I1 | One frame's dispatch error aborted whoever owned the shared drain | **SUPERSEDED structurally** — there is no shared caller-owned drain. Direct dispatch returns its own result; background dispatch reports per-frame errors without aborting receive progress, and AuthFetch routes correlated reply failures to the owning request |
...
| 3 | Inbound 30s wait deadlocks the pull-based drain loop; releasing message sits behind the blocked one | R2b | **FIXED structurally** — one sequential worker owns each session's general queue and may park at the gate; certificate responses bypass general workers on the independent control lane. The sole receiver and every other session remain available. Deterministic queued-general/certificate-response regression is green and control-through-worker mutation is red |
...
| 33 | Many tests called protocol futures untimed → a regression **hangs** rather than failing; `cargo test` has no per-test timeout | R2b | **FIXED after background-receive audit** — the three reopened bare awaits (`send_message`, spawned dispatch completion, and `Notify`) are explicitly bounded; the paused-time gate deadline test has no ready-loop spin and every background/dispatch rendezvous fails on a deadline |
...
| AuthFetch request headers | `HashMap<String, String>` | The resulting payload is signed, but the map itself is not serialized | **SAFE WITH REGISTERED DIVERGENCE** — normalizes, rejects case-insensitive duplicates that cannot retain TS insertion order, then applies the shared TS `localeCompare` comparator before encoding |
| Other wallet RPC maps (`PartialCertificate`, acquire/list/discovery args/results) | `IndexMap<...>` | No path into `AuthMessage`, but TS preserves JavaScript object insertion order on the observable WalletWire | **FIXED on the separate TS-wire-order axis** — five TS-generated, non-alphabetical vectors pin acquire fields, acquire subject keyring, prove fields, discovery attributes, and list-result keyring; decode→encode preserves each frame. This does not revise the earlier finding that these maps are outside auth preimages |
| `RemittanceCertificate.fields` / `keyring_for_verifier` | `IndexMap<String, String>` | No signature or hash covers the MessageBox envelope today, but `serde_json::to_string` serializes both maps into it | **HARDENED PROPHYLACTICALLY** — deterministic envelope bytes avoid leaving randomized serialized maps one future hop from a preimage |
...
| 40 | Transport receive/dispatch | Background `onData` callbacks | **One goroutine per connection**, sequential handler loop (`websocket_transport.go:74,112-129`) | **FIXED** — peer-owned background receiver; nonce-routed handshake waiters; one sequential general worker per session with a 64-frame queue; separate 16-control admission; 1024 live sessions with expired-first/LRU eviction; bounded error observer with frame type/request-ID metadata | Implemented on `feat/background-receive-task`. `process_next`/`process_pending`, the pull mutex, deferred queue, pending-response net, and 64-global/8-session message semaphores are deleted; session ordering/isolation, queue overflow, cap admission, lifecycle shutdown, and the certificate-release path are mutation-proven |
...
- **Receive architecture resolved** (#40): Rust now has a background receiver and
  per-session sequential general workers, so certificate-gated frames wait
  without a deferred queue, caller-driven pump, or shared general-dispatch
  permit. Control frames retain an independent bounded lane. Background errors
  are per-frame events, never session latches.
